let second = ``;
for (let i = 0; i < 60; i++) {
  let div = `<div id="second${
    i + 1
  }" style="font-size:20px;width:100%;text-align:right;position:absolute;display:inline-block;transform:rotate(${
    (i - 1) * -6
  }deg);">${i + 1} 秒</div>`;
  second = second + div;
}
...
function transformBox() {
  ...
  document.querySelector(".second-box").style.transform = `rotate(${
    second360 * 360 + (second - 1) * 6
  }deg)`;
  document.querySelector(".minute-box").style.transform = `rotate(${
    minute360 * 360 + (minute - 1) * 6
  }deg)`;
  document.querySelector(".hour-box").style.transform = `rotate(${
    hour360 * 360 + (hour - 1) * 15
  }deg)`;
  document.querySelector(".day-box").style.transform = `rotate(${
    day360 * 360 + (day - 1) * _deg
  }deg)`;
  document.querySelector(".month-box").style.transform = `rotate(${
    month360 * 360 + month * 30
  }deg)`;
  ...
  document.querySelector(`#minute${minute === 0 ? "60" : minute}`).classList =
    "now-date";
  document.querySelector(`#hour${hour === 0 ? "24" : hour}`).classList =
    "now-date";
